	...
	protected void process(final HttpServletRequest request,
		...
		for (String key : hubConfig.getAllParams().keySet()) {
			if (!server.getConfiguration().containsKey(key)) {
				server.getConfiguration().put(key,
						hubConfig.getAllParams().get(key));
			}
		}
		...
		reply(response, "ok");
		getRegistry().getGridMonitorMBean().handleRegistrationString(myRegistrationRequestString,"ok");
